...
export function Measure() {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[#0e0e11]">
      <header className="flex items-center gap-2 px-2 pt-6 pb-3">
        <button
          type="button"
          onClick={() => navigate("/")}
        ...
        <h1 className="text-lg font-bold text-white">내 소음 측정</h1>
      </header>

      <main className="flex flex-1 flex-col items-center px-6 pt-10">
        {status === "idle" && (
          <>
            <Mic size={40} className="mb-4 text-gray-500" />
            ...
            <button
              type="button"
              onClick={start}
              className="w-full max-w-xs rounded-xl bg-white py-3 text-sm font-semibold text-gray-900 active:bg-gray-200"
            >
              측정 시작
            </button>
          </>
        ...
        {status === "requesting" && <p className="text-sm text-gray-500">마이크 권한 요청 중...</p>}
        ...
        {status === "error" && (
          <>
            <p className="mb-6 text-center text-sm font-medium text-gray-200">
        ...
        {status === "active" && (
          <div className="flex w-full max-w-xs flex-col items-center">
            ...
            <p className="mt-2 text-xs text-gray-500">기기 마이크 기반 근사치입니다</p>
            ...
            <button
              ...
              className="mt-8 w-full rounded-xl bg-white py-3 text-sm font-semibold text-gray-900 active:bg-gray-200"
            >
              이 값을 열차에 기여
            </button>
          </div>
      ...
      {toastVisible && (
        <div className="fixed inset-x-0 bottom-8 flex justify-center px-6">
          <div className="rounded-full bg-white px-4 py-2 text-sm font-medium text-gray-900 shadow-lg">
            측정값이 반영되었어요 (데모)
          </div>
        </div>
      )}
