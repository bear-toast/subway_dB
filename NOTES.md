...
## 적용 로직: dB 등급 · 청력 보호 안전 상한

`SEGMENT_DB`(78, 84…) 자체는 SPEC.md 데모용 시드값이지만, 그 값을 **몇 dB부터 "시끄러움"으로 분류하고 이어폰 볼륨을 몇 %까지 안전 상한으로 둘지**의 경계값은 처음에 근거 없는 임의 수치(75/80dB, 70·60·55%)로 시작했다. 아래 공식 자료를 근거로 재조정해 코드(`src/data/line2.js`)에 반영했다.
...
**왜 소음이 심할수록 볼륨을 낮추라고 하는가 — 직관과 반대인 이유.** 소음을 뚫고 또렷하게 들으려면 보통 이어폰 출력이 주변 소음보다 10~15dB는 더 커야 한다(마스킹 효과). 즉 주변 소음이 85dB면 "잘 들리는" 수준까지 볼륨을 올릴 경우 출력은 95~100dB까지 치솟는데, NIOSH 기준 94dB의 안전 노출시간은 1시간, 100dB는 15분로 급락한다. **소음을 이기려고 볼륨을 올리는 바로 그 행동이, 정확히 청력 손상 위험 구간으로 밀어 넣는 행동**이라는 뜻이다. 그래서 이 값은 "이 정도면 잘 들려요"라는 권장치가 아니라 "이 이상은 넘기지 마세요"라는 **안전 상한**이며, 명료도보다 청력 보호를 우선한다 — 안 들리면 볼륨을 올리는 대신 노이즈캔슬링을 쓰거나 잠깐 포기하라는 것이 이 앱의 입장이다. 초기 버전은 이를 "권장 볼륨"이라는 문구로 표시해 "이 볼륨이 듣기에 적당하다"는 반대 의미로 오해될 수 있었기에, UI 문구를 **"청력 보호 안전 상한"**으로 바꿨다.
